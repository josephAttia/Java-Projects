...
//			another element might have the key "there" and the value ["thar"] 
   ...
   public Map<String, ArrayList<String>> flip()
   {
      ...
      for(String name : englishToPirate.keySet()){
         String englishName = englishToPirate.get(name);

         if(!ans.containsKey(englishName)){
            ArrayList<String> newNames = new ArrayList();
            ans.put(englishName, newNames);
            ans.get(englishName).add(name);
         }
         else{
            ans.get(englishName).add(name);
         }
